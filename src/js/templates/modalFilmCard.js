...
function onAddToWatchedToLocalStorage(event) {
  ...
  if (
    !event.target.hasAttribute('data-watched-delete') &&
    btnWatched?.classList.contains('btn-header-active')
  ) {
    wathedListInMyLibrary.insertAdjacentHTML(
      'beforeend',
      addFilmCardToList(dataObj)
    );
  }
...
function onAddToQueueToLocalStorage(event) {
  ...
  if (
    !event.target.hasAttribute('data-queue-delete') &&
    btnQueue?.classList.contains('btn-header-active')
  ) {
    queueListInMyLibrary.insertAdjacentHTML(
      'beforeend',
      addFilmCardToList(dataObj)
    );
  }
          ...
          film.poster_path
                ...
                <div class="film__description">
                    ...
                    <p class="film__title about">${
                      genresArr.length > 2
                        ? genresArr.slice(0, 2).join(', ') + ', Other'
                        : genresArr.join(', ')
                    }
                    | ${
                      film.release_date.split('-')[0]
                    } | <span class='vote'>${Number(film.vote_average).toFixed(
    1
  )}</span></p>
                </div>
			</li>
